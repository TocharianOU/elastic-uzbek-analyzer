...
Three corpora are used, and each measurement below names the one it was taken on:
the **source stem list** (42,869 distinct stems, the input to `roots.tsv`), the
**root lexicon** (65,727 entries, what the plugin ships), and the
**attested-form table** (37,762 inflected forms with their lemma).
...
Measured on the 42,869 distinct stems of the source stem list:
...
| `roots.tsv` | 65,727 | UzbekLemmaStems-POS-Dataset (Apache-2.0) + attested-table lemmas |
...
Measured over the root lexicon, 813 entries (1.41%) end in a core affix and
decompose to another root — and the two halves of that number behave completely differently:
...
over-decomposition of the root lexicon: 292 of 65,727 (0.44%). None of it is
unintended — 172 are the plural unifications just described, 95 are the
attested-form table resolving entries such as `berdi` to `ber`, and 25 are
allomorph readings such as `burni` to `burun`.
...
`telefon`, `xarita` and `qopqoq` out of VERB, and grew the lexicon by roughly
16,000 entries; the attested-form table later added 7,920 more, for 65,727.
